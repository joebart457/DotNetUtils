...
namespace CliParser
{
    internal class CliCommand
    {
        ...
        public string ToHelpString()
        {
            ...
            var maxAbbrievationMaxLength = ParametersByPosition.Max(x => x.Value.Abbreviation.Length);
            sb.AppendLine(string.Join("", ParametersByPosition.Values.Select(p => $"\n    {p.ToHelpString(maxAbbrievationMaxLength)}")));
            return sb.ToString();
        }
    ...
    internal class CliCommand<TResult>
    {
        private Dictionary<string, CommandParameter> ParametersByName { get; set; } = new Dictionary<string, CommandParameter>();
        private Dictionary<int, CommandParameter> ParametersByPosition { get; set; } = new Dictionary<int, CommandParameter>();
        public List<string> Path { get; set; }
        ...
        {
            if (methodInfo == null) throw new ArgumentNullException(nameof(methodInfo));
            if (methodInfo.ReturnType != typeof(TResult)) throw new InvalidOperationException($"command does not return requested type {typeof(TResult).Name}");
            _methodInfo = methodInfo;
            Path = GetPath(methodInfo);
            var parameters = GetParameters(methodInfo);
            parameters.ForEach(p =>
        ...
        public TResult Run<Ty>(string[] args)
        {
            var arguments = ParseArguments(args);
            var result = _methodInfo.Invoke(typeof(Ty), arguments.ToArgumentArray());
            if (result is Task task) task.Wait();
            if (result is TResult tresult) return tresult;
            throw new Exception($"unable to cast result to {typeof(TResult).Name}");
        }

        public TResult Run<Ty>(string[] args, Ty instance) where Ty : class
        {
            var arguments = ParseArguments(args);
            var result = _methodInfo.Invoke(instance, arguments.ToArgumentArray());
            if (result is Task task) task.Wait();
